...
export class AuthService {
  private createToken(currentTokneId: string): {
    accesToken: string;
    expiresIn: number;
  } {
  ...
  async login(req: authLoginDto, res: Response): Promise<any> {
    try {
      ...
      const isPasswordCorrect = await comparer(
        req.password,
        user.hash,
        user.iv,
        user.salt,
  ...
  async passwordChanging(
    ...
    try {
      const passValidation = await comparer(
        data.oldPassword,
        user.hash,
        user.iv,
        user.salt,
      ...
      if (data.newPassword === data.oldPassword) {
        return {
          actionStatus: false,
          message: 'Nowe hasło powinno różnić sie od obecnego',
        };
      }
  ...
  async emailChanging(
    ...
    try {
      const userValidation = await comparer(
        data.password,
        user.hash,
        user.iv,
        user.salt,
